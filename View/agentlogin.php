...
if (isset($_POST['login'])) {
    ...
    // ✅ FIX: Changed 'id' to 'agent_id'
    $stmt = $conn->prepare("SELECT agent_id, password FROM agents WHERE username = ?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    
    // ✅ FIX: Bind result to $agent_id instead of $id
    $stmt->bind_result($agent_id, $hashed_pass);
    
    if ($stmt->fetch() && password_verify($password, $hashed_pass)) {
        $_SESSION['agent_logged_in'] = true;
        
        // ✅ FIX: Save correct ID to session
